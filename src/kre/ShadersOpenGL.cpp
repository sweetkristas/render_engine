	...
	namespace OpenGL
	{
		namespace
		{
			...
			const char* const complex_vs = 
				...
				//"    vec4 pos = u_mv_matrix * (vec4(a_position, 0.0, 1.0)+delta);\n"
				//"    gl_Position = u_p_matrix * (pos);\n" //  + delta
				"    gl_Position = u_p_matrix * u_mv_matrix * vec4(a_position, 0.0, 1.0);\n"
				//"    v_normal = a_normal;\n"
				"}\n";
			const char* const complex_fs =
				...
				//"    float alpha = clamp(min(dist - (u_line_width - blur), u_line_width - dist) / blur, 0.0, 1.0);\n"
